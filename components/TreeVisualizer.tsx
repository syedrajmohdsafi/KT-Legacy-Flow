...
const TreeVisualizer: React.FC<TreeVisualizerProps> = ({ data, type, expandAll = false, isPrinting = false }) => {
  ...
  const prepareData = useCallback((node: Person, shouldExpand: boolean): ExtendedPerson => {
    const newNode = { ...node } as ExtendedPerson;
    newNode.isRevealed = shouldExpand;

    if (node.children && node.children.length > 0) {
      const processedChildren = node.children.map(child => prepareData(child, shouldExpand));
      if (shouldExpand) {
        newNode.children = processedChildren;
        newNode._children = null;
      } else {
        newNode._children = processedChildren;
  ...
  const [rootData, setRootData] = useState<ExtendedPerson>(() => {
    const root = prepareData(data, expandAll);
  ...
  useEffect(() => {
    const prepared = prepareData(data, expandAll);
  ...
  const zoomToFit = useCallback(() => {
    if (!svgRef.current || !containerRef.current || !currentHierarchy || !zoomRef.current) return;
    ...
    const yMin = d3.min(descendants, d => d.y) || 0;
    const yMax = d3.max(descendants, d => d.y) || 0;
    
    if (isPrinting) {
      // Use viewBox for printing - this is the most reliable way to handle scaling and centering
      const paddingX = 100; 
      const paddingYTop = 150; // Extra padding at top to prevent header overlap
      const paddingYBottom = 300; // Extra padding at bottom to prevent text cutoff
      const vBoxX = xMin - paddingX;
      const vBoxY = yMin - paddingYTop;
      const vBoxW = (xMax - xMin) + (paddingX * 2);
      const vBoxH = (yMax - yMin) + paddingYTop + paddingYBottom;
      
      // Safety check for valid numbers
        if (!isNaN(vBoxX) && !isNaN(vBoxY) && !isNaN(vBoxW) && !isNaN(vBoxH) && vBoxW > 0 && vBoxH > 0) {
          svg.attr('viewBox', `${vBoxX} ${vBoxY} ${vBoxW} ${vBoxH}`);
          svg.attr('preserveAspectRatio', 'xMidYMin meet');
          
          // Set physical dimensions so browser knows how to paginate
          svg.attr('width', vBoxW);
          svg.attr('height', vBoxH);
          
          svg.style('min-width', '0'); 
          svg.style('max-height', 'none');
          
          // Reset the transform on the group so it doesn't interfere with viewBox
          svg.select('g.main-group').attr('transform', 'translate(0,0) scale(1)');
        }
    } else {
      ...
      const treeWidth = xMax - xMin + 400;
      const treeHeight = yMax - yMin + 400;
      const scale = Math.min(width / treeWidth, height / treeHeight, 1.0);
      
      const transform = d3.zoomIdentity
        .translate(width / 2 - ((xMax + xMin) / 2) * scale, 100)
        .scale(scale);
        
      svg.call(zoomRef.current.transform, transform);
    }
  }, [currentHierarchy, isPrinting]);
  ...
  const renderTree = useCallback(() => {
    ...
    // Use more compact gaps for printing
    const horizontalGap = isPrinting ? 500 : (isMobile ? 1200 : 1350); 
    const verticalGap = isPrinting ? 250 : (isMobile ? 250 : 400);
    ...
    nodesMerged.transition().duration(isPrinting ? 0 : 500)
      .attr('transform', (d: any) => `translate(${d.x}, ${d.y})`)
      .style('opacity', 1);

    const circleRadius = isPrinting ? 40 : (isMobile ? 50 : 70);
    const spouseXOffset = isPrinting ? 80 : (isMobile ? 90 : 130);
    const fontSizeMain = isPrinting ? '20px' : (isMobile ? '28px' : '36px');
    const fontSizeLabel = isPrinting ? '14px' : (isMobile ? '18px' : '22px');
    const fontSizeRole = isPrinting ? '11px' : (isMobile ? '14px' : '16px');
  ...
  }, [rootData, isInitialRender, centerNode, isPrinting]);
  ...
  }, [renderTree, rootData, isPrinting]);
  ...
  useEffect(() => {
    if (isPrinting) {
      // Small delay to ensure renderTree has finished and currentHierarchy is updated
      const timer = setTimeout(() => {
        zoomToFit();
      }, 100);
      return () => clearTimeout(timer);
    } else {
      // Reset height and zoom when printing is done
      const svg = d3.select(svgRef.current);
      svg.attr('height', '100%');
      
      // Return to initial zoom state
      const timer = setTimeout(() => {
        resetZoom();
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [isPrinting, zoomToFit, resetZoom]);
  ...
  return (
    <div className={`w-full bg-[radial-gradient(circle_at_center,_#0f172a_0%,_#020617_100%)] print:bg-white print:bg-none ${isPrinting ? 'w-full h-auto relative overflow-visible' : 'h-full relative overflow-hidden'}`}>
      <div ref={containerRef} className={isPrinting ? "w-full h-auto relative" : "w-full h-full absolute inset-0"}>
        <svg ref={svgRef} className={isPrinting ? "w-full h-auto cursor-grab active:cursor-grabbing overflow-visible" : "w-full h-full cursor-grab active:cursor-grabbing"} style={{ touchAction: 'none' }} />
      </div>
