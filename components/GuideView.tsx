...
const WAZU_STEPS = [
  { 
    ...
    img: "images/wudu-wash-hands.png" 
  },
  { 
    name: "Mouth", 
    ...
    img: "images/wudu-rinse-mouth.png" 
  },
  { 
    name: "Nose", 
...
const SURAHS = [
  ...
  },
  {
    ...
    arabic: "اللَّهُمَّ إِنِّي ظَلَمْتُ نَفْسِي ظُلْمًا كَثِيرًا وَلَا يَغْفِرُ الذُّنُوبَ إِلَّا أنتَ فَاغْفِرْ لِي مَغْفِرَةً مِنْ عِنْدِكَ وَارْحَمْنِي إِنَّكَ أَنْتَ الْغَفُورُ الرَّحِيمُ . اَللّٰهُمَّ اغْفِرْلِیْ وَلِوَالِدَیَّ وَلِاَوْلَادِیْ وَلِجَمِیْعِ الْمُؤْمِنِیْنَ وَالْمُؤْمِنَاتِ وَالْمُسْلِمِیْنَ وَالْمُسْلِمَاتِ الْاَحْیَاءِ مِنْهُمْ وَالْاَمْوَاتِ . رَبَّنَا آتِنَا فِی الدُّنْیَا حَسَنَةً وَفِی الْآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ . سُبْحَانَ رَبِّكَ رَبِّ الْعِزَّةِ عَمَّا یَصِفُوْنَ وَسَلَامٌ عَلَى الْمُرْسَلِیْنَ وَالْحَمْدُ لِلّٰهِ رَبِّ الْعَالَمِیْنَ",
    roman: "Allahumma inni zalamtu nafsi zulman kathira wala yaghfiru dhunuba illa Anta faghfirli maghfiratan min 'Indika war-hamni innaka Antal Ghafurur Rahim. Allahummagh-firli wali-walidayya wali-awladi wal-jami'il mu'minina wal-mu'minati wal-muslimina wal-muslimatil ahya-i minhum wal-amwat. Rabbana atina fid-dunya hasanatan wa fil-akhirati hasanatan waqina 'adhaban-nar. Subhana Rabbika Rabbil 'izzati 'amma yasifun wa salamun 'alal-mursalin wal-hamdu lillahi Rabbil 'alamin."
  },
  {
    name: "Dua e qunoot",
...
const DAILY_PRAYERS = [
  ...
  { 
    name: "Dhuhr", 
    time: "After Noon", 
    rakats: "2 Wazu Dugana, 4 Sunnah, 4 Fard, 2 Sunnah", 
    niyyah: {
  ...
  { 
    name: "Isha", 
    time: "Night", 
    rakats: "2 Wazu Dugana, 4 Sunnah, 4 Fard, 2 Sunnah, 3 Witr", 
    niyyah: {
...
const GuideView: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'wazu' | 'prayers' | 'surahs' | 'chiragh'>('wazu');
  ...
  useEffect(() => {
    const updateWidth = () => {
      if (pdfContainerRef.current) {
        setPdfWidth(pdfContainerRef.current.getBoundingClientRect().width);
      }
    };
    updateWidth();
    window.addEventListener('resize', updateWidth);
    return () => window.removeEventListener('resize', updateWidth);
  }, [activeTab, selectedPdfIdx]);
  ...
  const currentPdfPath = [
    "documents/Chiragh-e-Deen-e-Mahdi-Roman.pdf",
    "documents/Chiragh-e-Deen-e-Mahdi-Urdu.pdf",
    "documents/Chirag-e-Deen-e-Nabawi.pdf",
    "documents/Lailatul-Qadr-Ke-Dugane-Ki-Niyyat.pdf",
    "documents/Taraweeh-ki-niyyat.pdf",
    "documents/Dua-after-Witr.pdf"
  ][selectedPdfIdx];
  ...
  return (
    <div className="max-w-5xl mx-auto w-full py-8 space-y-8 px-4 flex flex-col min-h-screen">
      ...
      <div className="flex bg-slate-900/80 p-1.5 rounded-2xl border border-white/5 backdrop-blur-xl shrink-0 shadow-2xl">
        {['wazu', 'prayers', 'surahs', 'chiragh'].map((tab) => (
          <button 
            key={tab}
          ...
          >
            {tab === 'wazu' ? 'Wazu' : tab === 'prayers' ? 'Prayers' : tab === 'surahs' ? 'Surahs' : 'Chiragh-e-Deen'}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-32">
        ...
        {activeTab === 'chiragh' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom duration-500 h-full flex flex-col">
            <div className="flex flex-row flex-nowrap overflow-x-auto pb-4 gap-4 snap-x scrollbar-hide shrink-0">
              {[
                { name: "Roman Urdu", file: "documents/Chiragh-e-Deen-e-Mahdi-Roman.pdf" },
                { name: "Urdu", file: "documents/Chiragh-e-Deen-e-Mahdi-Urdu.pdf" },
                { name: "Deen-e-Nabawi English", file: "documents/Chirag-e-Deen-e-Nabawi.pdf" },
                { name: "Lailatul Qadr", file: "documents/Lailatul-Qadr-Ke-Dugane-Ki-Niyyat.pdf" },
                { name: "Taraweeh", file: "documents/Taraweeh-ki-niyyat.pdf" },
                { name: "40- Allahuma Duas", file: "documents/Dua-after-Witr.pdf" }
              ].map((pdf, i) => (
                <button 
                  key={i} 
                  onClick={() => setSelectedPdfIdx(i)}
                  className={`relative glass rounded-2xl p-4 border transition-all text-left group overflow-hidden shrink-0 min-w-[140px] snap-center ${selectedPdfIdx === i ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 hover:border-white/20'}`}
                >
                  <h4 className="text-sm font-black text-white leading-tight">{pdf.name}</h4>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">PDF Document</p>
                  {selectedPdfIdx === i && <div className="absolute top-2 right-2 text-indigo-500"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5H7z"/></svg></div>}
                </button>
              ))}
            </div>

            <div className="flex-1 min-h-[600px] glass rounded-3xl overflow-hidden border border-white/10 shadow-2xl relative bg-slate-900 flex flex-col" ref={pdfContainerRef}>
              <div className="flex items-center justify-between p-4 border-b border-white/10 bg-slate-900/50 backdrop-blur-md">
                <div className="flex items-center gap-2">
                  <button onClick={handleZoomOut} disabled={pdfScale <= 0.5} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white disabled:opacity-50 transition-all" title="Zoom Out">
                  ...
                  <span className="text-xs font-bold text-slate-400 w-12 text-center">{Math.round(pdfScale * 100)}%</span>
                  <button onClick={handleZoomIn} disabled={pdfScale >= 3} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white disabled:opacity-50 transition-all" title="Zoom In">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" /></svg>
                  </button>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={handlePrint} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white transition-all" title="Print">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                  </button>
                  <a href={currentPdfPath} download className="p-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white transition-all" title="Download">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  </a>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar flex justify-center p-4">
                <Document
                  file={currentPdfPath}
                  onLoadSuccess={onDocumentLoadSuccess}
                  loading={<div className="text-indigo-400 p-8 flex items-center gap-3"><div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin"/> Loading PDF...</div>}
                ...
                >
                  {Array.from(new Array(numPages), (el, index) => (
                    <div key={`page_${index + 1}`} className="mb-4 shadow-xl">
                      <Page 
                        pageNumber={index + 1} 
                        width={pdfWidth > 0 ? Math.min(pdfWidth - 32, 800) : undefined}
                        scale={pdfScale}
